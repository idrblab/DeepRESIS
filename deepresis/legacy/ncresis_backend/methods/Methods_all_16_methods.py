...
from . import _01_ORF_code as ORF_code
from . import  _02_CTDcode as CTDcode
from . import  _03_Fickettcode as Fickettcode
from . import  _04_kmer_counts as kmer_counts
from . import  _06_proparcoder as proparcoder
from . import  _07_GCcounts as GCcounts
from . import  _08_edpfeature as edpfeature
from . import  _09_StopCodon as StopCodon
from . import  _10_onehot as onehot
from . import  _15_SparseEncoding as SparseEncod
from . import  _18_SStructure as SStructure
from . import  _19_CTDcoder_st_ph as CTDcoderstph

from .repDNA import ac as RNA_ac
from .repDNA import psenac as RNA_psenac
...
def switch_meth(fun, textPath):
    if fun == '1': # 'Open reading frame (1D)',4+20+7,但7是六聚体分数是RNA的特征
        ...
        return T1
    elif fun == '2_1': # Entropy Density Related,20
        EDP = edpfeature.EDPcoder(textPath).getEDP()        
        return EDP
    elif fun == '2_2': #  'Global descriptor (1D)',30
        CTD = CTDcode.CTDcoder(textPath).get_ctd()
        return CTD
    elif fun == '2_3': # 'K-mer (1D)',84
        ...
        return T1
    elif fun == '3': # 'Codon related (1D)',5,Fickett Score (1)+StopCod(4)
    ...
    elif fun == '6': # 'Pseudo protein related,5
        return proparcoder.ProtPar(textPath).get_protper()
    elif fun == '7': # 'Guanine-cytosine related (1D)',7
        return GCcounts.GCconder(textPath).get_gc()
    elif fun == '13': # One-hot encoding (2D)',1000×4
        return onehot.Onehot(textPath).get_onehot(1000) #2D 方法
    elif fun == '15': # 'Sparse encoding (2D)',1000×3,DNA和RNA通用，类似one hot
        return SparseEncod.get_encoding(textPath, 1000)
    elif fun == '16': # Structure-based Features,,1000×7
    ...
    elif fun == '17': # 'Nucleotide related (1D)',182
        ...
        return T1
    elif fun == '18': # 'Secondary structure (1D)',8
        ...
        return SStruc_T
    elif fun == '18_1': # 'EIIP based spectrum (1D)',8
        ...
        return SStruc_T
    elif fun == '19_1': # 'Solubility lipoaffinity (1D)',可溶性脂亲和力,42
        ACGT_encode = ['1000']
        ...
        return T1
    elif fun == '19_101': # 'Partition coefficient (1D)',分配系数,42
        ACGT_encode = ['0100']
        ...
        return T1
    elif fun == '19_2': # 'Polarizability refractivity (1D)',极化折射率,27
        ACGT_encode = ['0101']
        ...
        return T1
    elif fun == '19_3': # 'Hydrogen bond related (1D)',氢键,93
        ...
        return T1
    elif fun == '19_4': # 'Topological indice (1D)',拓扑指数,66
        ACGT_encode = ['0001']
        ...
        return T1
    elif fun == '19_5': # 'Molecular fingerprint (1D)',分子指纹,27
        ACGT_encode = ['0011']
        T1 = CTDcoderstph.CTDcoder(textPath,ACGT_encode).get_ctd()
